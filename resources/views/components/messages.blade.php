@if (session()->has('message'))
    <div x-data="{ show: true }" x-show="show" x-init="() => { setTimeout(() => { show = false; }, 5000); }" @click="show = false"
        class="z-50 fixed right-5 top-5 bg-teal-100 border-t-4  border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md"
        role="alert">
        <div class="flex">
...
@if (session()->has('custom-error'))
    <div x-data="{ show: true }" x-show="show" x-init="() => { setTimeout(() => { show = false; }, 5000); }"
        class="z-50 fixed right-5 top-5 bg-red-100 border-t-4  border-red-500 rounded-b text-red-900 px-4 py-3 shadow-md"
        role="alert">
        <div class="flex">
            ...
            <div>
                <p class="font-bold">{{ session('custom-error') }}</p>
            </div>
        </div>
    </div>
@endif
...
@if ($errors->any())
    <div x-data="{ show: true }" x-show="show" x-init="() => { setTimeout(() => { show = false; }, 5000); }"
        ...
        <div class="text-red-900  " role="alert">
            <div class="flex">
                <div class="py-1"><svg class="fill-current h-4 w-4 text-red-500 mr-2"
